...
im = Image.open(str(groundTruth.filename))
imc = Image.open(str(lowSample.filename))
# load the photos
...
while True:
    ...
    choice = int(input("Enter 1 for Sample, 2 for Total, 3 for more info: "))
    if choice == 3:
        print(
            "Sample takes random samples from the reference, and cross references its image pair. Fast but can lack accuracy")
...
if choice == 2:
    ...
    while i < pixel_total:
        while xc < x:
            r, g, b = im.getpixel((xc, yc))
            rc, gc, bc = imc.getpixel((xc, yc))
